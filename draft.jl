...
function externalContractPolicyUpdate!(arr_Mu, arr_cStar, arr_dummiesStar, mat_Mu, mat_Πe, vec_Vi, mat_Ve; para = para_de, sol_uc = sol_uc_de, bounds = bounds_de)
    ...
    for i_ind in 1:num_i, e_ind in 1:num_e, iota_ind in 1:num_i 
        ...
        internalMatchingValue_e = mat_Ve[i_ind, e_ind]
        ...
        if ieInternalMatch && eiotaInternalMatch && internalMatchingValue_e < ve_val                    
            ...
            if externalContract.flag # if matching with higher mat_Ve, then update with the new contract
                arr_Mu[i_ind, e_ind] = true
                arr_cStar[i_ind, e_ind] = externalContract.c_star
                arr_dummiesStar[i_ind, e_ind] = externalContract.dummies_star
            else # matching is not possible with this higher ve_val, the pair should get their search value which is
                arr_Mu[i_ind, e_ind] = false
                arr_cStar[i_ind, e_ind] = -1.0
                arr_dummiesStar[i_ind, e_ind] = [0,0,0]
            end
        end
    end
